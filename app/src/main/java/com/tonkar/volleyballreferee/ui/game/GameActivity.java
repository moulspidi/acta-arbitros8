...
public class GameActivity extends AppCompatActivity 
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            // Go back to edit teams: save current game as setup and open GameSetupActivity
            StoredGamesService storedGamesService = new StoredGamesManager(this);
            if (mGame != null) {
                storedGamesService.saveSetupGame(mGame);
            }
            Intent setupIntent = new Intent(this, com.tonkar.volleyballreferee.ui.setup.GameSetupActivity.class);
            setupIntent.putExtra("edit_current", true);
            startActivity(setupIntent);
            finish();
            return true;
        } else if (item.getItemId() == R.id.action_close_match) {
            // Confirm and close current match so it no longer appears in Resume
            new androidx.appcompat.app.AlertDialog.Builder(this, R.style.AppTheme_Dialog)
                .setTitle(R.string.close_match)
                .setMessage(getString(R.string.navigate_home_question))
                ...
                .show();
            return true;
        } else if (item.getItemId() == R.id.action_game_action_menu) {
            showGameActionMenu();
            return true;
        } else {
